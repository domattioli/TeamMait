"""Lightweight RAG + Chat CLI for TeamMait

This file is a Streamlit-free refactor of the previous interactive app. It exposes:
- query(prompt, model, ...): performs Chroma retrieval, builds a system prompt, and calls the Chat provider.
- a small CLI entrypoint for ad-hoc queries.

Environment variables expected (preferred):
- OPENAI_API_KEY for OpenAI
- ANTHROPIC_API_KEY for Anthropic (if using Claude)

Notes:
- chroma DB path is ./rag_store (keeps existing store)
- loads supporting documents from doc/RAG and doc/RAG/supporting_documents
"""
...
This is a Streamlit-free refactor of the previous interactive app. It exposes:
- query(prompt, model, ...): performs Chroma retrieval, builds a system prompt, and calls the Chat provider.
- a small CLI entrypoint for ad-hoc queries.
